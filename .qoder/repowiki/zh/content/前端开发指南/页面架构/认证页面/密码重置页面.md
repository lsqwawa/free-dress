# 密码重置页面

<cite>
**本文档引用的文件**
- [ForgotPasswordScreen.tsx](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx)
- [ResetPasswordScreen.tsx](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx)
- [auth.ts](file://FreeDressApp/src/api/auth.ts)
- [auth.controller.ts](file://backend/src/modules/auth/auth.controller.ts)
- [auth.service.ts](file://backend/src/modules/auth/auth.service.ts)
- [captcha.service.ts](file://backend/src/modules/auth/captcha.service.ts)
- [reset-password.dto.ts](file://backend/src/modules/auth/dto/reset-password.dto.ts)
- [RootNavigator.tsx](file://FreeDressApp/src/navigation/RootNavigator.tsx)
- [Input.tsx](file://FreeDressApp/src/components/Input.tsx)
- [Button.tsx](file://FreeDressApp/src/components/Button.tsx)
- [index.ts](file://FreeDressApp/src/types/index.ts)
- [jwt-auth.guard.ts](file://backend/src/common/guards/jwt-auth.guard.ts)
- [jwt.strategy.ts](file://backend/src/modules/auth/strategies/jwt.strategy.ts)
- [main.ts](file://backend/src/main.ts)
</cite>

## 更新摘要
**所做更改**
- 更新了JWT令牌管理和认证系统的实现细节
- 添加了新的认证流程和令牌刷新机制
- 更新了安全最佳实践和令牌管理策略
- 增强了错误处理和用户引导机制

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

本文档详细介绍了畅搭(FreeDress)应用中密码重置功能的完整实现，包括忘记密码页面(ForgotPasswordScreen)和重置密码页面(ResetPasswordScreen)的设计与实现。该功能涵盖了从手机号验证、验证码获取到新密码设置的完整用户体验流程，并实现了基于JWT令牌的现代认证系统和严格的安全最佳实践。

## 项目结构

密码重置功能涉及前端React Native应用和后端NestJS服务的协作，主要文件分布如下：

```mermaid
graph TB
subgraph "前端应用 (FreeDressApp)"
FP[ForgotPasswordScreen.tsx<br/>忘记密码页面]
RP[ResetPasswordScreen.tsx<br/>重置密码页面]
API[auth.ts<br/>认证API封装]
NAV[RootNavigator.tsx<br/>导航配置]
INPUT[Input.tsx<br/>输入组件]
BTN[Button.tsx<br/>按钮组件]
TYPES[index.ts<br/>类型定义]
end
subgraph "后端服务 (backend)"
AC[auth.controller.ts<br/>认证控制器]
AS[auth.service.ts<br/>认证服务]
CS[captcha.service.ts<br/>验证码服务]
DTO[reset-password.dto.ts<br/>DTO验证]
JWT_GUARD[jwt-auth.guard.ts<br/>JWT认证守卫]
JWT_STRATEGY[jwt.strategy.ts<br/>JWT策略]
MAIN[main.ts<br/>应用入口]
end
FP --> API
RP --> API
API --> AC
AC --> AS
AS --> CS
AS --> JWT_GUARD
AS --> JWT_STRATEGY
AC --> DTO
MAIN --> JWT_GUARD
MAIN --> JWT_STRATEGY
```

**图表来源**
- [ForgotPasswordScreen.tsx:1-304](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L1-L304)
- [ResetPasswordScreen.tsx:1-231](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L1-L231)
- [auth.controller.ts:1-106](file://backend/src/modules/auth/auth.controller.ts#L1-L106)

**章节来源**
- [ForgotPasswordScreen.tsx:1-304](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L1-L304)
- [ResetPasswordScreen.tsx:1-231](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L1-L231)
- [auth.ts:1-101](file://FreeDressApp/src/api/auth.ts#L1-L101)

## 核心组件

密码重置功能由两个核心页面组成，每个页面都有其特定的功能职责：

### 忘记密码页面 (ForgotPasswordScreen)
- **手机号验证**：验证用户输入的手机号格式
- **验证码系统**：集成图片验证码获取和验证
- **令牌获取**：向后端请求密码重置令牌
- **页面导航**：成功后跳转到重置密码页面

### 重置密码页面 (ResetPasswordScreen)
- **密码强度验证**：确保新密码符合安全要求
- **密码确认**：验证两次输入的密码一致性
- **密码更新**：使用重置令牌更新用户密码
- **用户反馈**：提供成功和失败的明确反馈

**章节来源**
- [ForgotPasswordScreen.tsx:44-115](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L44-L115)
- [ResetPasswordScreen.tsx:42-93](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L42-L93)

## 架构概览

密码重置功能采用前后端分离的架构设计，实现了完整的JWT认证流程：

```mermaid
sequenceDiagram
participant U as 用户
participant FP as 忘记密码页面
participant API as 前端API
participant AC as 认证控制器
participant AS as 认证服务
participant CS as 验证码服务
participant JWT as JWT服务
participant DB as 数据库
U->>FP : 输入手机号和验证码
FP->>API : 请求验证码
API->>AC : GET /auth/captcha
AC->>CS : 生成验证码
CS->>DB : 存储验证码(内存)
CS-->>AC : 返回SVG验证码
AC-->>API : 返回验证码数据
API-->>FP : 显示验证码
U->>FP : 提交手机号验证
FP->>API : forgotPassword(phone, captchaId, captchaAnswer)
API->>AC : POST /auth/forgot-password
AC->>AS : 验证手机号和验证码
AS->>CS : 验证验证码
CS->>DB : 检查验证码有效性
AS->>DB : 查询用户信息
AS->>JWT : 生成重置令牌
AS->>DB : 存储重置令牌
AS-->>AC : 返回重置令牌
AC-->>API : 返回重置令牌
API-->>FP : 导航到重置密码页面
U->>RP : 输入新密码
RP->>API : resetPassword(resetToken, newPassword)
API->>AC : POST /auth/reset-password
AC->>AS : 验证重置令牌
AS->>DB : 检查令牌有效性
AS->>DB : 加密并更新密码
AS-->>AC : 返回成功消息
AC-->>API : 返回成功响应
API-->>RP : 显示成功提示
```

**图表来源**
- [ForgotPasswordScreen.tsx:76-115](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L76-L115)
- [ResetPasswordScreen.tsx:73-93](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L73-L93)
- [auth.controller.ts:55-68](file://backend/src/modules/auth/auth.controller.ts#L55-L68)

## 详细组件分析

### 忘记密码页面 (ForgotPasswordScreen)

#### 页面布局和动画
页面采用渐进式动画展示，标题和表单分别有不同的进入动画效果：

```mermaid
flowchart TD
Start([页面加载]) --> LoadCaptcha[加载验证码]
LoadCaptcha --> AnimateTitle[标题动画]
AnimateTitle --> AnimateForm[表单动画]
AnimateForm --> Ready[等待用户输入]
Ready --> ValidatePhone{验证手机号}
ValidatePhone --> |无效| ShowPhoneError[显示手机号错误]
ValidatePhone --> |有效| ValidateCaptcha{验证验证码}
ValidateCaptcha --> |无效| ShowCaptchaError[显示验证码错误]
ValidateCaptcha --> |有效| SubmitRequest[提交验证请求]
SubmitRequest --> Loading[显示加载状态]
Loading --> Success{请求成功?}
Success --> |是| NavigateToReset[导航到重置页面]
Success --> |否| ShowError[显示错误信息]
ShowError --> ReloadCaptcha[重新加载验证码]
ReloadCaptcha --> Ready
```

**图表来源**
- [ForgotPasswordScreen.tsx:60-74](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L60-L74)
- [ForgotPasswordScreen.tsx:95-115](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L95-L115)

#### 表单验证逻辑
页面实现了多层次的表单验证：

1. **手机号格式验证**：使用正则表达式验证中国手机号格式
2. **验证码验证**：确保用户输入了验证码
3. **实时反馈**：通过Alert组件提供即时错误提示

#### 验证码系统集成
- **SVG验证码**：后端生成SVG格式的图片验证码
- **验证码存储**：使用内存存储验证码答案和元数据
- **自动刷新**：用户可以点击验证码区域重新获取

**章节来源**
- [ForgotPasswordScreen.tsx:44-115](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L44-L115)
- [ForgotPasswordScreen.tsx:153-193](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L153-L193)

### 重置密码页面 (ResetPasswordScreen)

#### 密码验证规则
页面实施严格的密码安全策略：

```mermaid
flowchart TD
Input[用户输入新密码] --> CheckEmpty{密码为空?}
CheckEmpty --> |是| EmptyError[显示空密码错误]
CheckEmpty --> |否| CheckLength{长度>=6?}
CheckLength --> |否| ShortError[显示长度错误]
CheckLength --> |是| CheckMatch{两次输入匹配?}
CheckMatch --> |否| MismatchError[显示不匹配错误]
CheckMatch --> |是| SubmitRequest[提交重置请求]
SubmitRequest --> Loading[显示加载状态]
Loading --> Success{重置成功?}
Success --> |是| ShowSuccess[显示成功提示]
Success --> |否| ShowFailure[显示失败原因]
ShowFailure --> Ready[保持页面可用]
ShowSuccess --> NavigateHome[导航到首页]
```

**图表来源**
- [ResetPasswordScreen.tsx:73-93](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L73-L93)

#### 密码安全策略
- **最小长度**：密码至少6位字符
- **最大长度**：密码不超过20位字符
- **重复验证**：要求用户确认新密码
- **加密存储**：后端使用bcrypt进行密码哈希

**章节来源**
- [ResetPasswordScreen.tsx:42-93](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L42-L93)

### JWT认证系统

#### JWT令牌管理
新的认证系统实现了完整的JWT令牌管理机制：

```mermaid
flowchart TD
Login[用户登录] --> GenerateTokens[生成访问令牌和刷新令牌]
GenerateTokens --> StoreRefresh[存储刷新令牌]
StoreRefresh --> ReturnTokens[返回令牌给客户端]
Access[访问受保护资源] --> ValidateAccess{验证访问令牌}
ValidateAccess --> |有效| AllowAccess[允许访问]
ValidateAccess --> |无效| CheckRefresh{检查刷新令牌}
CheckRefresh --> |有效| GenerateNewAccess[生成新访问令牌]
GenerateNewAccess --> ReturnNewAccess[返回新令牌]
CheckRefresh --> |无效| RedirectLogin[重定向到登录]
```

#### JWT策略实现
JWT策略负责验证令牌并提取用户信息：

```mermaid
classDiagram
class JwtStrategy {
-AuthService authService
-Strategy strategy
+constructor(authService)
+validate(payload) Promise<User>
}
class JwtAuthGuard {
+canActivate(context) boolean
+handleRequest(err, user, info) any
}
class AuthService {
+generateTokens(userId, phone) Promise<Tokens>
+refreshTokens(userId, phone) Promise<Tokens>
+validateUser(userId) Promise<User>
}
JwtStrategy --> AuthService : "验证用户"
JwtAuthGuard --> JwtStrategy : "继承"
AuthService --> JwtStrategy : "使用"
```

**图表来源**
- [jwt.strategy.ts:10-39](file://backend/src/modules/auth/strategies/jwt.strategy.ts#L10-L39)
- [jwt-auth.guard.ts:8-22](file://backend/src/common/guards/jwt-auth.guard.ts#L8-L22)

#### 安全配置
应用启动时进行关键环境变量验证：

```mermaid
flowchart TD
Startup[应用启动] --> CheckEnvVars{检查环境变量}
CheckEnvVars --> |缺少| FatalError[致命错误并退出]
CheckEnvVars --> |完整| CheckSecretLength{检查JWT密钥长度}
CheckSecretLength --> |<16位| SecurityWarning[安全警告并退出]
CheckSecretLength --> |>=16位| StartServer[启动服务器]
```

**章节来源**
- [auth.service.ts:143-161](file://backend/src/modules/auth/auth.service.ts#L143-L161)
- [jwt.strategy.ts:12-21](file://backend/src/modules/auth/strategies/jwt.strategy.ts#L12-L21)
- [main.ts:14-25](file://backend/src/main.ts#L14-L25)

### 后端服务实现

#### 验证码服务 (CaptchaService)
验证码服务实现了多项安全防护机制：

```mermaid
classDiagram
class CaptchaService {
-Map~string, CaptchaEntry~ store
-Map~string, RateLimitEntry~ rateLimit
-number CAPTCHA_TTL
-number MAX_ATTEMPTS
-number RATE_LIMIT_WINDOW
-number RATE_LIMIT_MAX
+generate(ip) CaptchaData
+verify(captchaId, answer) boolean
-generateRandomText(length) string
-generateSvg(text) string
-checkRateLimit(ip) void
-cleanup() void
}
class CaptchaEntry {
+string answer
+number createdAt
+number attempts
}
class RateLimitEntry {
+number count
+number firstRequest
}
CaptchaService --> CaptchaEntry : "存储"
CaptchaService --> RateLimitEntry : "限流"
```

**图表来源**
- [captcha.service.ts:31-122](file://backend/src/modules/auth/captcha.service.ts#L31-L122)

#### 认证服务 (AuthService)
认证服务处理密码重置的核心业务逻辑：

```mermaid
flowchart TD
ForgotPassword[忘记密码请求] --> VerifyCaptcha[验证验证码]
VerifyCaptcha --> CheckUser{用户存在?}
CheckUser --> |不存在| UserError[抛出用户不存在异常]
CheckUser --> |存在| GenerateResetToken[生成重置令牌]
GenerateResetToken --> StoreResetToken[存储重置令牌]
StoreResetToken --> ReturnResetToken[返回重置令牌]
ResetPassword[重置密码请求] --> VerifyResetToken[验证重置令牌]
VerifyResetToken --> CheckToken{令牌有效?}
CheckToken --> |无效| TokenError[抛出令牌无效异常]
CheckToken --> |有效| CheckExpiry{检查令牌过期}
CheckExpiry --> |过期| ExpiredError[抛出令牌过期异常]
CheckExpiry --> |有效| HashPassword[加密新密码]
HashPassword --> UpdatePassword[更新数据库]
UpdatePassword --> MarkUsed[标记令牌已使用]
MarkUsed --> Success[返回成功]
```

**图表来源**
- [auth.service.ts:180-207](file://backend/src/modules/auth/auth.service.ts#L180-L207)
- [auth.service.ts:214-242](file://backend/src/modules/auth/auth.service.ts#L214-L242)

**章节来源**
- [captcha.service.ts:31-122](file://backend/src/modules/auth/captcha.service.ts#L31-L122)
- [auth.service.ts:180-242](file://backend/src/modules/auth/auth.service.ts#L180-L242)

## 依赖关系分析

密码重置功能的依赖关系体现了清晰的分层架构：

```mermaid
graph TB
subgraph "表现层"
FP[ForgotPasswordScreen]
RP[ResetPasswordScreen]
INPUT[Input组件]
BTN[Button组件]
end
subgraph "API层"
API[auth.ts]
AXIOS[axios.ts]
end
subgraph "业务逻辑层"
AC[AuthController]
AS[AuthService]
end
subgraph "基础设施层"
CS[CaptchaService]
PRISMA[PrismaService]
JWT[JwtService]
BC[Bcrypt]
JWT_GUARD[JwtAuthGuard]
JWT_STRATEGY[JwtStrategy]
end
FP --> API
RP --> API
API --> AXIOS
API --> AC
AC --> AS
AS --> CS
AS --> PRISMA
AS --> JWT
AS --> JWT_GUARD
AS --> JWT_STRATEGY
CS --> PRISMA
AS --> BC
JWT_GUARD --> JWT_STRATEGY
```

**图表来源**
- [auth.ts:1-101](file://FreeDressApp/src/api/auth.ts#L1-L101)
- [auth.controller.ts:19-22](file://backend/src/modules/auth/auth.controller.ts#L19-L22)
- [auth.service.ts:30-37](file://backend/src/modules/auth/auth.service.ts#L30-L37)

**章节来源**
- [auth.ts:1-101](file://FreeDressApp/src/api/auth.ts#L1-L101)
- [auth.controller.ts:19-22](file://backend/src/modules/auth/auth.controller.ts#L19-L22)

## 性能考虑

### 前端性能优化
- **动画性能**：使用react-native-reanimated实现流畅的页面过渡动画
- **内存管理**：验证码和重置令牌使用内存存储，定期清理过期数据
- **网络优化**：API调用包含加载状态指示，避免用户重复提交

### 后端性能优化
- **验证码缓存**：使用Map数据结构存储验证码，查询时间复杂度O(1)
- **定时清理**：后台定时任务清理过期验证码和重置令牌
- **数据库索引**：用户表按手机号建立索引，提高查询效率
- **JWT缓存**：令牌验证使用内存缓存，减少数据库查询

## 故障排除指南

### 常见问题及解决方案

#### 验证码相关问题
- **验证码过期**：验证码有效期2分钟，需要重新获取
- **验证码错误**：最多允许3次验证尝试，超过限制需重新获取
- **请求过于频繁**：IP限流每分钟最多10次请求

#### 密码重置问题
- **重置令牌失效**：重置令牌有效期10分钟，过期后需要重新申请
- **密码不符合要求**：新密码必须6-20位且两次输入一致
- **网络连接问题**：检查网络状态，重试操作

#### JWT令牌问题
- **令牌过期**：访问令牌默认7天有效期，需要使用刷新令牌
- **刷新令牌无效**：刷新令牌30天有效期，过期后需要重新登录
- **环境变量配置错误**：JWT_SECRET和JWT_REFRESH_SECRET必须配置且长度≥16位

#### 错误处理机制
页面实现了完善的错误处理：
- **输入验证错误**：立即显示具体的错误信息
- **网络请求错误**：捕获异常并提供友好的错误提示
- **业务逻辑错误**：根据具体错误类型提供相应的解决方案
- **JWT认证错误**：自动重定向到登录页面

**章节来源**
- [ForgotPasswordScreen.tsx:95-115](file://FreeDressApp/src/screens/ForgotPasswordScreen.tsx#L95-L115)
- [ResetPasswordScreen.tsx:73-93](file://FreeDressApp/src/screens/ResetPasswordScreen.tsx#L73-L93)

## 结论

畅搭(FreeDress)应用的密码重置功能实现了完整的用户体验流程，具有以下特点：

### 设计优势
- **用户体验友好**：清晰的步骤指导和即时反馈
- **安全性强**：多层验证和防护机制，基于JWT的现代化认证
- **技术实现优雅**：前后端分离架构，模块化设计，支持令牌刷新

### 安全最佳实践
- **验证码防护**：2分钟有效期、3次尝试限制、IP限流
- **令牌管理**：10分钟有效期的重置令牌，7天访问令牌，30天刷新令牌
- **密码保护**：bcrypt加密存储，严格的密码强度要求
- **防重放攻击**：一次性重置令牌机制和过期控制
- **JWT安全**：环境变量配置验证，密钥长度安全检查

### 技术亮点
- **动画体验**：流畅的页面过渡和交互反馈
- **错误处理**：完善的错误提示和恢复机制
- **可扩展性**：模块化的架构设计，便于功能扩展
- **现代化认证**：完整的JWT令牌生命周期管理

该密码重置功能为用户提供了安全、便捷的账户恢复体验，同时为开发者提供了清晰的代码结构和良好的可维护性。新的JWT认证系统进一步增强了应用的安全性和用户体验。